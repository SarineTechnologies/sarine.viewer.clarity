...
Write-Output "Copy assets files..."
& "$env:ModulesPath\s3sync.ps1" -EnvName $envName -envId $envId -envType $envType -WebSiteFolder $iisAppName -targetPathWithoutEnvPrefix $targetPathWithoutEnvPrefix -maxAge 31536000 -sourceRelativePath $sourceRelativePath


if($envType -eq "prod"){
	$siteUri = "http://viewer-api.sarine.com/cache/v2/atom/sarine.viewer.clarity"
}
else{
	$siteUri = "http://viewer-api-$envId.$envType.sarine.com/cache/v2/atom/sarine.viewer.clarity"
}
